...
const EXPIRED_SUBTITLE = "?∏Ï¶ùÍ∏∞Í∞Ñ??ÎßåÎ£å??Î≤àÌò∏?ÖÎãà??";
const EXPIRED_DETAIL = "?∏Ï¶ùÍ∏∞Í∞Ñ??ÎßåÎ£å??Î≤àÌò∏?ÖÎãà?? ÏßÅÏ†ë ?ïÏù∏ Î∂Ä?ÅÎìúÎ¶ΩÎãà??";

/** ÏΩ??§Î≤Ñ?àÏù¥ Ï≤??òÏù∏?∏Ïö© ??live ?∞ÏÑ†, ?ÜÏúºÎ©?editor. includeDigitalCard ?¨Ìï® */
...
/** CEO Í∏∞Î≥∏ VLUE Î∏åÎûú??Î°úÍ≥† (?ÖÎ°ú???ÜÏùÑ ?? */
...
/**
 * ?§Î≤Ñ?àÏù¥ ?¨Î¶º Ï°∞Ìöå ??export_snapshot_json ?µÏß∏ SELECT ?ÜÏù¥ ?ÅÌò∏Îß?JSON path.
 * business_cards.company_name ??ÎπÑÏñ¥ ?àÎäî CEO¬∑Í∞úÏù∏Î™ÖÌï®(VCID KOREA ?? Î≥¥Ï†ï.
 */
...
/** ?úÎìú¬∑ÎØ∏Î¶¨Î≥¥Í∏∞??Î∏åÎûú???êÎ¶¨?úÏãú ???§Ï†ú ?ÅÌò∏Î°??°Ï∂ú?òÏ? ?äÏùå */
...
type ExportSnapLite = {
  ...
  logoUrl: string;
  /** ?àÏñ¥Î°?Î∞∞Í≤Ω Ï¥àÏ†ê: top | center | bottom */
  photoFocus: string;
  noTitlePhoto: boolean;
};

/**
 * export_snapshot_json ?ÑÏ≤¥ SELECT Í∏àÏ? ??Shared Pooler egress ??£º Î∞©Ï?.
 * JSON path Î°?ÏßßÏ? ?çÏä§???ÑÎìúÎß?Ï∂îÏ∂ú.
 */
...
/** ?ºÏ??¥Ïä§ ?°Ï∂ú???∞ÎùΩÏ≤???User.email + snap lite + profileJson */
...
type LookupOptions = {
  ...
  /**
   * Í≥µÍ∞ú Í≥µÏú† ÎßÅÌÅ¨(OG/Ïπ¥Ïπ¥???§ÌÅ¨?òÌçº)??
   * Í≤Ä?â¬∑ÌåîÎ°úÏõå ÎπÑÍ≥µÍ∞?ÎßàÏä§?πÏùÑ ?ÅÏö©?òÏ? ?äÍ≥† Î™ÖÌï® ?úÍ∏∞Î™ÖÏùÑ Í∑∏Î?Î°??∏Ï∂ú?úÎã§.
   * (ÎßÅÌÅ¨Î•?Î≥¥ÎÇ∏ ?¨Îûå?Ä ?¥Î? Ï¥àÎ? ?ÑÏùò ???åÎπÑÍ≥µÍ∞ú ?åÏõê?çÏúºÎ°?Í∞ÄÎ¶¨Î©¥ ????
   */
  forPublicOgShare?: boolean;
  /** ?µÌôî ?§Î≤Ñ?àÏù¥ ??Í∏∞Í? DB¬∑export ?§ÎÉÖ?∑ÏùÑ Í±¥ÎÑà?∞Í≥† ÏµúÏÜå ?ÑÎìúÎ°?Ï°∞Ìöå */
  forCallOverlay?: boolean;
  /** ?åÏä§???úÎ??àÏù¥????normal | abnormal */
...
async function lookupCardForCallOverlay(raw: string, opts: LookupOptions) {
  ...
  if (!e164) {
    return { status: 400 as const, body: { error: "?†Ìö®??Î≤àÌò∏ ?ïÏãù???ÑÎãô?àÎã§.", matched: false } };
  }
  ...
  if (card && card.verificationStatus === "approved") {
    ...
    /* ?∞ÎùΩ¬∑?åÍ∞ú???∏Ï¶ù ?¨Î??Ä Î¨¥Í??òÍ≤å ÎßàÏä§???¥Î∞± (?¥ÏÑ† ?§ÎÉÖ??ÎπÑÏñ¥ ?àÏñ¥??Î™ÖÌï® ?ïÎ≥¥ ?†Ï?) */
    ...
    const exportSnap = {
      ...
      /* ?∏Ï¶ù Î≤àÌò∏: ÎßàÏä§??DigitalCardÍ∞Ä ?Ä?¥Ì?¬∑Î°úÍ≥† ?ïÎ≥∏ (?ºÏù∏ ?§ÎÉÖ ?îÏû¨ Î¨¥Ïãú) */
...
/** Î≤àÌò∏ Í∏∞Ï? Ï°∞Ìöå ?ëÎãµ Î≥∏Î¨∏ ??GET /lookup ¬∑ GET /by-number Í≥µÏö© */
export async function lookupCardByRawNumber(raw: string, opts: LookupOptions = {}) {
  ...
  if (billingGate.gate === "unmatched") {
    /* ?†Ïòà Í≤ΩÍ≥º¬∑?¥Ï? ?åÏÑ†?Ä ?ºÎ∞ò ÎØ∏Ïù∏Ï¶ùÏúºÎ°?Ï∑®Í∏â ???ÑÎûò user/unmatched Î∂ÑÍ∏∞Î°?*/
  } else {
  const card = await prisma.businessCard.findFirst({
  ...
  if (billingGate.gate === "unmatched") {
    return {
      status: 404 as const,
      body: { matched: false, message: "?±Î°ù??Î™ÖÌï®???ÜÏäµ?àÎã§." }
    };
  }
  ...
  if (user) {
    ...
    const logoOnly =
      ...
      null;
    const isCeo = isPlatformCeoHandle(user.publicHandle);
    /* ?ÑÎ°ú???¨ÏßÑÎß?image_url. CEO Î°úÍ≥† ?¨Î°ØÎß?VLUE Í∏∞Î≥∏. ?Ä??Îπ??¨ÏßÑ ??null(?§Î£®?? */
