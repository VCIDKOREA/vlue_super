...
function blockedReasonForFeature(featureType: FeatureType, gateCode: string) {
  ...
  if (featureType === "room_command" || featureType === "post_desc" || featureType === "web_ppt") {
...
/** AI 기능 선체크(카운터 미증가) — PPT/post_desc 등 작업 시작 전 */
vmingRoutes.post("/feature/check", async (c) => {
  ...
  const featureType: FeatureType = body.featureType || "web_ppt";
