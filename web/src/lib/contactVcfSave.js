...
function tryNativeContactBridge(vcf, filename) {
  ...
  try {
    ...
    if (window.VlueAndroid?.saveContact) {
      window.VlueAndroid.saveContact(vcf, filename);
      return true;
    }
  } catch {
    /* ignore */
...
export async function saveProfileToDeviceContacts(profile) {
  ...
  if (tryNativeContactBridge(vcf, filename)) {
