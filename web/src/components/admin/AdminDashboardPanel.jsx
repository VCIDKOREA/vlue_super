...
import {
  ...
  fetchJobOccupationPendingReviews,
...
function JobOccupationReviewTab({ onToast }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const q = await fetchJobOccupationPendingReviews();
  ...
  const resolve = async (reviewId, action) => {
    ...
    try {
      ...
      await resolveJobOccupationReview(reviewId, action);

      onToast?.(action === "approve" ? "직업인증 승인 완료" : "직업인증 반려");
  ...
  return (
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-3">
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <p className="text-[13px] font-black text-slate-900">직업인증 서류 대기</p>
      ...
      {error ? <p className="mb-2 text-[11px] font-bold text-red-600">{error}</p> : null}

      {!loading && requests.length === 0 ? (

        <p className="rounded-lg bg-slate-50 py-8 text-center text-[12px] text-slate-500">대기 중인 직업인증 서류가 없습니다.</p>
        ...
        <ul className="space-y-3">
          ...
          {requests.map((r) => (
            ...
            <li key={r.id} className="rounded-lg border border-slate-100 bg-slate-50/80 p-3">

              <p className="text-[13px] font-black text-slate-900">{r.displayLabel || r.occupationLabel}</p>

              <p className="mt-0.5 text-[11px] text-slate-500">

                {r.occupationId} · {r.docKind} · {r.docFileName || "서류"}
...
export default function AdminDashboardPanel({ onToast }) {
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      {dashTab === "job-occupation" ? <JobOccupationReviewTab onToast={onToast} /> : null}
