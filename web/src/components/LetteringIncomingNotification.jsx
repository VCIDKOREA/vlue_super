...
import {
  shouldShowPeerVlueVerifiedSeal,
...
import ShowcaseDccBottomBanner from "./ads/ShowcaseDccBottomBanner.jsx";
...
export default function LetteringIncomingNotification({
  ...
  /*
   * 청록 인증마크 — 본인 미리보기만 localStorage 폴백.
   * 상대 통화/빅푸시는 상대 카드 배지 + verified 만 (수신자 배지 절대 금지).
   */
  const peerBadgeExplicit = c.vlueVerifiedBadge ?? c.vlue_verified_badge;
  const isOwnShowcasePreview = Boolean(previewMode && showOwnerSettings);
  const officialBadgeActive = isOwnShowcasePreview
    ? shouldShowVlueVerifiedSeal({ vlueVerifiedBadge: peerBadgeExplicit })
    : shouldShowPeerVlueVerifiedSeal({
        verified,
        vlueVerifiedBadge: peerBadgeExplicit,
        profileKind: c?.profileKind,
        contactSafeCare: isContactSafeCare
      });
