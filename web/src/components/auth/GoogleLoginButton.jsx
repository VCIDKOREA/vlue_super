import { apiUrl } from "../../lib/apiBase.js";

/**
 * Google 로그인 버튼 — 서버 OAuth: GET /api/v1/auth/google
 */
export default function GoogleLoginButton({ className = "", disabled = false, onBeforeNavigate }) {
  const startGoogleOAuth = () => {
    if (disabled) return;
    onBeforeNavigate?.();
    window.location.assign(apiUrl("/api/v1/auth/google"));
  };

  return (
    <button
      type="button"
      disabled={disabled}
      onClick={startGoogleOAuth}
      aria-label="Google로 로그인"
      className={[
        "flex w-full items-center justify-center gap-2 rounded-md border border-slate-200 bg-white py-3 text-[15px] font-semibold text-slate-800 transition",
        "hover:bg-slate-50 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-50",
        className
      ]
        .filter(Boolean)
        .join(" ")}
    >
      <svg className="h-5 w-5 shrink-0" viewBox="0 0 48 48" aria-hidden>
        <path
          fill="#FFC107"
          d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.1 8 3l5.7-5.7C34.2 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.5-.4-3.5z"
        />
        <path
          fill="#FF3D00"
          d="M6.3 14.7l6.6 4.8C14.7 16.1 19 13 24 13c3.1 0 5.8 1.1 8 3l5.7-5.7C34.2 6.1 29.4 4 24 4 16.3 4 9.6 8.3 6.3 14.7z"
        />
        <path
          fill="#4CAF50"
          d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.3 35.2 26.8 36 24 36c-5.3 0-9.7-3.3-11.3-7.9l-6.6 5.1C9.5 39.6 16.2 44 24 44z"
        />
        <path
          fill="#1976D2"
          d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.3 4.1-4.1 5.5l.1.1 6.2 5.2C39.2 37.3 44 32.5 44 24c0-1.3-.1-2.5-.4-3.5z"
        />
      </svg>
      <span>Google로 로그인</span>
    </button>
  );
}
