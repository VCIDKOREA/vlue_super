...
const FAQS = [
  ...
  },
  {
    ...
    a: '보통은 재설치가 답이 아닙니다. VLUE는 유심 자체가 아니라, 가입 시 PASS 본인인증으로 확인된 전화번호·계정에 묶입니다. 유심을 바꾸면 기기·통신사 번호가 달라져 통화·발신번호·본인인증 번호와 맞지 않아 오류가 날 수 있습니다. 같은 사람이 번호만 변경한 경우 → 앱 재설치보다 설정 → 전화번호(본인인증 번호) 안내를 확인한 뒤, 본인인증 후 고객센터(support@vlue.kr)로 번호 갱신을 요청하세요. 다른 사람 유심으로 테스트한 경우 → 그 번호로 별도 테스트 계정을 쓰거나, 기존 계정과 번호가 달라 에러가 나는 것이 정상입니다. 앱이 꼬인 것 같다면 앱 데이터 삭제·재로그인 정도면 충분하며, 서버에 저장된 계정·번호는 그대로 유지됩니다.',
  },
  {
    q: '같은 계정으로 기기만 바꿨을 때는 어떻게 하나요?',
    a: '같은 VLUE 계정·같은 본인인증 번호라면 보통 새 기기에 앱을 설치한 뒤 로그인하면 됩니다. 재가입·초기화는 필요 없습니다. 새 기기에서도 본인인증에 쓴 번호의 유심(또는 그 번호로 본인인증 가능한 상태)이 맞는지 확인하세요. 번호는 그대로인데 기기만 교체한 경우와, 번호까지 바꾼 경우는 다릅니다. 번호 변경은 위 FAQ(유심·번호 변경)와 같이 본인인증 후 고객센터 절차를 따릅니다. Google·카카오 등 연동 로그인을 썼다면 새 기기에서도 동일 연동으로 로그인할 수 있습니다.',
  },
  {
    q: '휴대폰 번호를 바꿨을 때 VLUE에서 해야 할 일은?',
