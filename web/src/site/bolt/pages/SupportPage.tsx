...
const FAQS = [
  {
    q: 'VLUÉ V1 멤버십은 어떻게 가입하나요?',
    a: '상단 [인증신청]에서 무료·유료(월 9,900원)·B2B 요금제를 비교한 뒤, VLUÉ 앱에서 가입·결제합니다. 웹에서는 요금 안내와 신청 안내를 제공합니다.',
  },
  {
    q: '앱에서 「기본 전화 앱」으로 VLUÉ를 설정하면 일반 전화가 안 되나요?',
    a: '아니요. 통신사 일반 전화(걸기·받기·요금)는 그대로입니다. Android에서 VLUÉ를 기본 전화 앱으로 지정하면, 통화가 연결됐을 때 VLUÉ가 통화 화면(디지털 인증명함·블루 쇼케이스)을 보여 주고 키패드·종료 제어를 돕습니다. 전화 회선 자체를 막는 설정이 아닙니다. iPhone은 기본 전화 앱을 바꿀 수 없어 오버레이 방식으로 동작합니다. 자세한 안내는 [서비스소개] → 설치형 앱 → 「기본 전화 앱 설정」을 참고하세요.',
  },
  {
  ...
  {
    q: '유심만 바꿨는데 오류가 납니다. 앱을 지우고 다시 설치해야 하나요?',
    a: '보통은 재설치가 답이 아닙니다. VLUÉ는 유심 자체가 아니라, 가입 시 PASS 본인인증으로 확인된 전화번호·계정에 묶입니다. 유심을 바꾸면 기기·통신사 번호가 달라져 통화·발신번호·본인인증 번호와 맞지 않아 오류가 날 수 있습니다. 같은 사람이 번호만 변경한 경우 → 앱 재설치보다 설정 → 전화번호(본인인증 번호) 안내를 확인한 뒤, 본인인증 후 고객센터(support@vlue.kr)로 번호 갱신을 요청하세요. 다른 사람 유심으로 테스트한 경우 → 그 번호로 별도 테스트 계정을 쓰거나, 기존 계정과 번호가 달라 에러가 나는 것이 정상입니다. 앱이 꼬인 것 같다면 앱 데이터 삭제·재로그인 정도면 충분하며, 서버에 저장된 계정·번호는 그대로 유지됩니다.',
  },
  {
    q: '같은 계정으로 기기만 바꿨을 때는 어떻게 하나요?',
    a: '같은 VLUÉ 계정·같은 본인인증 번호라면 보통 새 기기에 앱을 설치한 뒤 로그인하면 됩니다. 재가입·초기화는 필요 없습니다. 새 기기에서도 본인인증에 쓴 번호의 유심(또는 그 번호로 본인인증 가능한 상태)이 맞는지 확인하세요. 번호는 그대로인데 기기만 교체한 경우와, 번호까지 바꾼 경우는 다릅니다. 번호 변경은 위 FAQ(유심·번호 변경)와 같이 본인인증 후 고객센터 절차를 따릅니다. Google·카카오 등 연동 로그인을 썼다면 새 기기에서도 동일 연동으로 로그인할 수 있습니다.',
  },
  {
    q: '휴대폰 번호를 바꿨을 때 VLUÉ에서 해야 할 일은?',
    a: '앱을 지우고 다시 깔아도 계정에 등록된 본인인증 번호는 자동으로 바뀌지 않습니다. 설정 → 전화번호에서 「본인인증에 사용된 번호」을 확인한 뒤, 새 번호로 PASS 본인인증을 준비하고 고객센터(support@vlue.kr)에 회원 ID(또는 가입 휴대폰)·변경할 번호를 알려 번호 갱신을 요청하세요. 갱신 전·후에는 쇼케이스·통화 화면 등에 표시되는 연락처가 본인인증 번호와 일치하는지 확인해 주세요.',
  },
];
      ...
      <div className="bg-primary-600 py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-white/80 text-sm font-semibold">VLUÉ 고객지원</span>
          </div>
