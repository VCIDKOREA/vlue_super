...
const RESPONSES: [RegExp, string][] = [
  [/보이스피싱|사기|피해/, '보이스피싱 피해가 의심되시나요? 즉시 해당 전화를 끊고, 금융감독원(1332) 또는 경찰청(112)에 신고하세요. VLUÉ 검색창에서 해당 기관 번호를 확인하실 수 있습니다.'],
  [/인증|vlue인증|vlue 인증/, 'VLUÉ V1 멤버십은 일반(무료), 유료(월 9,900원·정가 28,300원 65% 특별 할인), B2B 풀 패키지(직원 회선 이벤트 5,200원)입니다. [인증신청] 또는 서비스소개에서 비교하세요.'],
  [/검색|기관확인|조회/, '검색창에 기관명, 전화번호 또는 사업자번호를 입력하면 공공데이터와 VLUÉ 인증 데이터를 동시에 비교하여 결과를 보여드립니다.'],
  ...
  [/유심|심카드|번호\s*변경|기기\s*변경|휴대폰\s*교체|폰\s*교체|재설치/, '유심·번호·기기만 바꿨을 때 보통 앱 재설치는 필요 없습니다. VLUÉ는 가입 시 본인인증한 전화번호·계정에 묶입니다. 같은 계정·같은 번호로 기기만 바꾸면 새 기기에 설치 후 로그인하면 됩니다. 번호를 바꾼 경우 설정→전화번호 안내를 본 뒤 본인인증 후 고객센터(support@vlue.kr)로 번호 갱신을 요청하세요. 자세한 내용은 [고객지원] FAQ를 참고하세요.'],
  [/연락|전화|이메일/, 'VLUÉ 고객센터: 1588-0000 (평일 09:00~18:00) / support@vlue.kr 로 문의하시면 됩니다.'],
  [/안녕|반가워|안녕하세요/, '안녕하세요! VLUÉ AI 고객센터입니다. 보이스피싱 예방, 기관 인증, 서비스 이용에 관해 무엇이든 도와드릴게요.'],
...
function getBotResponse(input: string): string {
  ...
  return '안녕하세요! VLUÉ AI 고객센터입니다. 보이스피싱 예방, 기관 인증 조회, 서비스 이용 방법 등 궁금하신 점을 말씀해 주세요. 더 자세한 상담은 고객센터(1588-0000)로 연락 주시기 바랍니다.';
}
        ...
        <div className="w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-fade-in-up" style={{ height: '460px' }}>
          <div className="flex items-center justify-between px-4 py-3 bg-primary-600">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-white text-sm font-semibold">VLUÉ AI 고객센터</p>
