...
export type ReferralChannel = "friend" | "promo";
...
export const REFERRAL_CHANNEL_LABELS: Record<
  ReferralChannel,
  { title: string; codeLabel: string; qualification: string }
> = {
  friend: {
    title: "지인 추천",
    codeLabel: "추천인 전화번호",
...
export function inferReferralChannelFromCode(raw: string | null | undefined): ReferralChannel | null {
  const t = String(raw || "").trim();
  if (!t) return null;
  const digits = t.replace(/\D/g, "");
  if (digits.length >= 10 && digits.length <= 13) return "friend";
  return "promo";
}
 ...
 */
export function effectiveSettlementChannel(
  attributionChannel: ReferralChannel,
  sponsorVluerPromoActive: boolean
): ReferralChannel {
  if (sponsorVluerPromoActive) return "promo";
  return attributionChannel;
}

export function minPaidReferralsForSponsorReward(channel: ReferralChannel): number {
